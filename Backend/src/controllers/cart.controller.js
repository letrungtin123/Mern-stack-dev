...
export const cartController = {
	...
	addCart: async (req, res) => {
		...
		// check product có tồn tại trong giỏ hàng hay chưa
		...
		if (productExitInCarts && productExitInCarts.length > 0) {
			...
			if (itemExist) {
				...
				// trường hợp chưa có sản phẩm nào trong giỏ hàng
			} else {
				// thêm sản phẩm vào giỏ hàng
				carts.push(product);
	...
	getCartByUserId: async (req, res) => {
		...
		let query = {};
		//kiểm tra role của user và check params có là 1 object rỗng không
		...
		//lấy giỏ hàng của user
		const result = await cartService.getCartByUserId(query, params);
		if (!result) {
			return (
	...
	asyncToCart: async (req, res) => {
		...
		userExist = await checkUserExist(userId);
		if (!userexist) {
		...
		//lấy giỏ hàng của user nếu cart is existed
		const { carts } = result;

		//check product in cart is existed? or check productId (size and color)
